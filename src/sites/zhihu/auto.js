...
const selectorMap = {
    prev: '#button-prev-chapter > a',
    next: '#button-next-chapter > a',
}
...
async function run(direction, interval, isScroll) {
    ...
    const sectionEl = document.querySelector('.reader-container > .reader-chapter-content > div > section')
    ...
    if (btn.classList.contains('disabled')) {
        ...
        if (retry <= 10) {
            retry++
            window.wrx_common_utils.log('页面没有加载完毕，2秒后重试')
            setTimeout(() => {
                run(direction, interval, isScroll)
            }, 2000)
        } else {
            window.wrx_common_utils.log('页面加载超时')
            isRunning = false
            setAutoRunningStatus(isRunning)
        }
        return
